...
class LBaaSv2AgentRPC(object):
    ...
    @log_helpers.log_method_call
    def rebuild_loadbalancer(self, context, loadbalancer, service, host):
        topic = '%s.%s' % (self.topic, host)
        return self.cast(
            context,
            self.make_msg(
                'rebuild_loadbalancer',
                loadbalancer=loadbalancer,
                service=service
            ),
            topic=topic)
    ...
    @log_helpers.log_method_call
    def create_acl_group(self, context, acl_group, host):
        topic = '%s.%s' % (self.topic, host)
        return self.cast(
            context,
            self.make_msg(
    ...
    @log_helpers.log_method_call
    def delete_acl_group(self, context, acl_group, host):
        topic = '%s.%s' % (self.topic, host)
        return self.cast(
            context,
            self.make_msg(
